...
Configuration for Optical Character Recognition (OCR) and PDF text extraction from uploaded images and documents. Search results surface matches found inside attached PDFs/images alongside a text excerpt.
...
<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`tesseract`</td></tr>
</table>
...
<table>
<tr><td>type</td><td>`[]string`</td></tr>
<tr><td>default</td><td>`deu,eng`</td></tr>
</table>
...
<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`pdftoppm`</td></tr>
</table>
...
<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`gemini-3.5-flash-lite`</td></tr>
</table>
...
<table>
<tr><td>type</td><td>`string`</td></tr>
<tr><td>default</td><td>`gemini-embedding-001`</td></tr>
</table>
...
Storyden can expose Google Drive folders to members through its own interface, so files are browsed, previewed and downloaded without leaving the site and without a Google account.

Access is granted through a Google Cloud service account. Create one with the Drive API enabled, download its JSON key, and share each folder you want to expose with the service account's email address. Administrators then register those folders in the admin panel.

The feature stays dormant until either of the credential values below is set.
